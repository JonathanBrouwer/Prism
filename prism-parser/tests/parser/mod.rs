...
macro_rules! parse_test {
    (name: $name:ident syntax: $syntax:literal passing tests: $($input_pass:literal => $expected:literal)* failing tests: $($input_fail:literal $(=> $errors:literal)?)*) => {
        ...
        fn $name() {
            ...
            let got = run_parser_rule::<SetError, _>(&grammar, "start", input, |v, _| v.to_debug_string(input)).unwrap_or_eprint();
            ...
            match run_parser_rule::<SetError, _>(&grammar, "start", input, |v, _| v.to_debug_string(input)) {
                Ok(got) => {
                    println!("Got: {:?}", got);
                    panic!();
                }
