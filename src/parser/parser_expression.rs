...
pub fn parse_expression<'a>(input: &'a [LexerItem<'a>]) -> Result<(Expression, &'a [LexerItem<'a>]), ParseError> {
    let (mut res, mut input) = one_or_more(input, |input|
        alt(input, vec![
            ...
            |input| parse_expression_control(input),
        ])
    )?;
...
pub fn parse_expression_control<'a>(input: &'a [LexerItem<'a>]) -> Result<(Expression, &'a [LexerItem<'a>]), ParseError> {
    let (name, input) = expect_control(input)?;
    Ok((Expression::Control(name), input))
}
