...
pub enum LambdayTerm<Sym: Eq + Hash + Clone> {
    ...
    FunType(Span, Rc<Self>, Rc<Self>),
    FunConstr(Span, Sym, Rc<Self>, Rc<Self>),
    FunDestr(Span, Rc<Self>, Rc<Self>),

    ProdType(Span, Vec<Rc<Self>>),
    ProdConstr(Span, Rc<Self>, Vec<Rc<Self>>),
    ProdDestr(Span, Rc<Self>, usize),

    SumType(Span, Vec<Rc<Self>>),
    SumConstr(Span, Rc<Self>, usize, Rc<Self>),
    SumDestr(Span, Rc<Self>, Rc<Self>, Vec<Rc<Self>>)
}
...
impl<Sym: Eq + Hash + Clone> LambdayTerm<Sym> {
    ...
    fn type_check_internal(&self, names: &mut HashMap<Sym, LambdayTerm<Sym>>) -> Result<LambdayTerm<Sym>, ()> {
        match self {
            ...
            FunConstr(span, sym, arg_type, body) => {
                ...
                //Function type
                Ok(FunType(*span, Rc::new((**arg_type).clone()), Rc::new(body_type)))
            }
        ...
        match self {
            ...
            FunDestr(span, fun, arg) => {
                ...
                match &fun {
                    ...
                    _ => FunDestr(*span, Rc::new(fun.clone()), arg.clone()),
                }
            }
            ...
            ProdDestr(span, val, num) => {
                ...
                match &val {
                    ...
                    _ => ProdDestr(*span,Rc::new(val.clone()), *num)
                }
            }
            ...
            SumDestr(span, val, into_type, opts) => {
                ...
                match &val {
                    ...
                    _ => SumDestr(*span,Rc::new(val.clone()), Rc::new(into_type.normalize_head()), opts.clone())
                }
            }
        }
    }
    ...
    fn substitute(&self, name: &Sym, to: &LambdayTerm<Sym>) -> LambdayTerm<Sym> {
        match self {
            ...
            FunType(span, arg_type, body_type) => {
                FunType(*span,Rc::new(arg_type.substitute(name, to)), Rc::new(body_type.substitute(name, to)))
            }
            FunConstr(span, sym, arg_type, body) => {
                if *sym == *name {
                    FunConstr(*span,sym.clone(), Rc::new(arg_type.substitute(name, to)), Rc::new((**body).clone()))
                } else {
                    FunConstr(*span, sym.clone(), Rc::new(arg_type.substitute(name, to)), Rc::new(body.substitute(name, to)))
                }
            }
            FunDestr(span, fun, arg) => {
                FunDestr(*span, Rc::new(fun.substitute(name, to)), Rc::new(arg.substitute(name, to)))
            }
            ProdType(span, types) => {
                ProdType(*span, types.into_iter().map(|l| Rc::new(l.substitute(name, to))).collect())
            }
            ProdConstr(span, typ, values) => {
                ProdConstr(*span,Rc::new(typ.substitute(name, to)), values.into_iter().map(|l| Rc::new(l.substitute(name, to))).collect())
            }
            ProdDestr(span, val, num) => {
                ProdDestr(*span, Rc::new(val.substitute(name, to)), *num)
            }
            SumType(span, types) => {
                SumType(*span, types.into_iter().map(|l| Rc::new(l.substitute(name, to))).collect())
            }
            SumConstr(span, typ, num, val) => {
                SumConstr(*span,Rc::new(typ.substitute(name, to)), *num, Rc::new(val.substitute(name, to)))
            }
            SumDestr(span, val, into_type, options) => {
                SumDestr(*span,Rc::new(val.substitute(name, to)), Rc::new(into_type.substitute(name, to)), options.into_iter().map(|l| Rc::new(l.substitute(name, to))).collect())
            }
        }
    }
}
